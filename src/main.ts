...
import { allottedTime, allottedTimeRange, deriveOutputs, elapsedSeconds, effectiveStage, formatAllottedTimeRange, isGraceStage, persistableTimerRun, recoverTimerRun, reduceTimer, type PresetSnapshot, type Stage, type TimerAction, type TimerClock, type TimerRun } from './timer';
...
type HistoryEntry = { id: string; presetTitle: string; speaker: string; club: string; duration: number; allottedTime?: number; allottedTimeStart?: number; allottedTimeEnd?: number; customAllottedTime?: boolean; stoppedAt: number; startedAt?: number; savedAt: string; variance: number; result: string; resultColor?: string };
const MAX_STAGES = 7;
...
function samePreset(a: PresetSnapshot, b: PresetSnapshot): boolean { return a.name === b.name && a.speaker === b.speaker && a.club === b.club && a.allottedTimeStart === b.allottedTimeStart && a.allottedTimeEnd === b.allottedTimeEnd && a.duration === b.duration && JSON.stringify(a.stages) === JSON.stringify(b.stages); }
...
function connectionMessage(): string { return navigator.onLine ? 'Network connection available' : 'Network connection unavailable · Running from the cached shell'; }
function connectionBadgeMarkup(): string { const online = navigator.onLine; const message = connectionMessage(); return `<span class="connection-badge ${online ? 'online' : 'offline'}" id="connection-badge" role="status" aria-live="polite" aria-label="${message}" title="${message}" data-shell-ready="${shellReady}"><i aria-hidden="true"></i></span>`; }
...
function formatHistoryAllottedTime(item: HistoryEntry): string { return Number.isFinite(item.allottedTimeStart) && Number.isFinite(item.allottedTimeEnd) ? (item.customAllottedTime ? `${formatTime(item.allottedTimeStart!)} to ${formatTime(item.allottedTimeEnd!)}` : formatAllottedTimeRange({ start: item.allottedTimeStart!, end: item.allottedTimeEnd! })) : formatTime(item.allottedTime ?? item.duration); }
function historyRowsMarkup(rows = displayedHistory()): string { return rows.length ? rows.map((item) => `<tr><td>${escapeHtml(item.presetTitle)}</td><td>${escapeHtml(item.club || '—')}</td><td>${escapeHtml(item.speaker)}</td><td>${item.startedAt ? new Date(item.startedAt).toLocaleString() : '—'}</td><td>${formatTime(item.duration)}</td><td>${formatHistoryAllottedTime(item)}</td><td>${formatTime(item.stoppedAt)}</td><td>${formatVariance(item.variance)}</td><td><span class="result-badge" style="--result-color:${/^#[0-9a-f]{6}$/i.test(item.resultColor ?? '') ? item.resultColor : '#71859e'}">${escapeHtml(item.result)}</span></td><td><button type="button" class="text-button history-delete" data-history-id="${item.id}">Delete</button></td></tr>`).join('') : '<tr><td colspan="10" class="history-empty">No matching timers.</td></tr>'; }
...
function importedPreset(value: unknown, usedIds: Set<string>): Preset {
  ...
  (['allottedTimeStart', 'allottedTimeEnd'] as const).forEach((key) => {
    if (item[key] === undefined) return;
    if (!Number.isInteger(item[key]) || Number(item[key]) < 0) throw new Error('Preset allotted times must be non-negative whole numbers of seconds.');
    preset[key] = Number(item[key]);
  });
...
function syncCurrentFromForm(): void {
  ...
  const optionalTime = (boundary: 'from' | 'to'): number | undefined => {
    const minutesText = value(`#allotted-time-${boundary}-minutes`)?.trim() ?? '';
    const secondsText = value(`#allotted-time-${boundary}-seconds`)?.trim() ?? '';
    if (!minutesText && !secondsText) return undefined;
    const minutes = minutesText === '' ? 0 : Number(minutesText);
    const seconds = secondsText === '' ? 0 : Number(secondsText);
    return Number.isInteger(minutes) && Number.isInteger(seconds) && minutes >= 0 && seconds >= 0 && seconds < 60 ? minutes * 60 + seconds : Number.NaN;
  };
  current.name = value('#preset-name') ?? current.name;
  current.speaker = value('#speaker') ?? current.speaker;
  current.club = value('#club') ?? current.club;
  current.allottedTimeStart = optionalTime('from');
  current.allottedTimeEnd = optionalTime('to');
  document.querySelectorAll<HTMLElement>('.stage-row').forEach((row, index) => {
    const get = (field: string) => row.querySelector<HTMLInputElement | HTMLSelectElement>(`[data-field="${field}"]`);
    const stage = current.stages[index];
    if (!stage) return;
    const minutes = get('threshold-minutes')?.value ?? '';
    const seconds = get('threshold-seconds')?.value ?? '';
    ...
    stage.threshold = Number.isInteger(parsedMinutes) && Number.isInteger(parsedSeconds) && parsedMinutes >= 0 && parsedSeconds >= 0 && parsedSeconds < 60 ? parsedMinutes * 60 + parsedSeconds : 0;
    stage.color = get('color')?.value ?? stage.color;
    stage.blink = (get('blink') as HTMLInputElement | null)?.checked ?? false;
    stage.grace = (get('grace') as HTMLInputElement | null)?.checked ?? false;
    stage.buzzer = (get('buzzer')?.value ?? stage.buzzer) as Stage['buzzer'];
  });
  const lastStageTime = current.stages[current.stages.length - 1]?.threshold ?? 0;
  const durationInput = value('#duration') ?? '';
...
function dispatch(action: TimerAction): void {
  if (action.type !== 'tick') timerSavedMessage = '';
...
function updateWarnings(): void { const target = document.querySelector<HTMLElement>('#timer-warnings'); if (!target) return; const warnings: string[] = []; if (activeRun?.state === 'running' && document.hidden) warnings.push('This page is hidden; keep it visible for reliable timing.'); if (activeRun?.state === 'running' && wakeLockWarning) warnings.push('Screen wake lock is unavailable or was released.'); if (serial.status.state !== 'connected') warnings.push('TimeLight device is not connected.'); if (activeRun?.clockWarning) warnings.push(activeRun.clockWarning); target.innerHTML = `${timerSavedMessage ? `<div class="timer-saved">${escapeHtml(timerSavedMessage)}</div>` : ''}${warnings.map((warning) => `<div class="timer-warning">${escapeHtml(warning)}</div>`).join('')}`; }
...
function stopAndSave(): void {
  ...
  const customAllottedStart = Number.isFinite(run.preset.allottedTimeStart) ? run.preset.allottedTimeStart : undefined;
  const customAllottedEnd = Number.isFinite(run.preset.allottedTimeEnd) ? run.preset.allottedTimeEnd : undefined;
  const hasCustomAllottedTime = customAllottedStart !== undefined || customAllottedEnd !== undefined;
  const allottedRange = hasCustomAllottedTime ? null : allottedTimeRange(run.preset, endAt);
  const customRange = customAllottedStart !== undefined && customAllottedEnd !== undefined ? { allottedTimeStart: customAllottedStart, allottedTimeEnd: customAllottedEnd } : {};
  history.unshift({ id: crypto.randomUUID(), presetTitle: run.preset.name || 'Untitled preset', speaker: speakerName(run.preset), club: clubName(run.preset), duration: endAt, allottedTime: customAllottedStart ?? customAllottedEnd ?? allottedTime(run.preset, endAt), ...customRange, ...(allottedRange ? { allottedTimeStart: allottedRange.start, allottedTimeEnd: allottedRange.end } : {}), ...(hasCustomAllottedTime ? { customAllottedTime: true } : {}), startedAt, stoppedAt, savedAt: new Date().toISOString(), variance: endAt - stoppedAt, result, resultColor: run.preset.stages[stageIndex]?.color });
  persistHistory();
  timerSavedMessage = 'Timer stopped and saved to history.';
  updateWarnings();
}
function exportHistoryCsv(): void {
  const csvCell = (value: string | number) => `"${String(value).replace(/"/g, '""')}"`;
  const rows = [['Preset title', 'Club name', 'Speaker name', 'Start date and time', 'Total duration', 'Allotted Time', 'Saved stop time', 'Time remaining', 'Result', 'Saved at'], ...displayedHistory().map((item) => [item.presetTitle, item.club, item.speaker, item.startedAt ? new Date(item.startedAt).toISOString() : '', formatTime(item.duration), formatHistoryAllottedTime(item), formatTime(item.stoppedAt), formatVariance(item.variance), item.result, item.savedAt])];
...
function bindEvents(): void {
  ...
  document.querySelector('#delete-preset')?.addEventListener('click', () => { if (!confirm('Delete this preset?')) return; presets = presets.filter((preset) => preset.id !== current.id); persistPresets(); current = structuredClone(presets[0] ?? starter); expandedStage = current.stages[0] ?? null; revertTarget = structuredClone(current); saved = presets.length > 0; render(); });
...
registerSW({ immediate: true, onOfflineReady: () => { shellReady = true; updateConnectionUi(); } });
