...
type HistoryEntry = { id: string; presetTitle: string; speaker: string; club: string; duration: number; stoppedAt: number; startedAt?: number; savedAt: string; variance: number; result: string; resultColor?: string };
...
function syncCurrentFromForm(): void { const value = (selector: string) => document.querySelector<HTMLInputElement>(selector)?.value; current.name = value('#preset-name') ?? current.name; current.speaker = value('#speaker') ?? current.speaker; current.club = value('#club') ?? current.club; document.querySelectorAll<HTMLElement>('.stage-row').forEach((row, index) => { const get = (field: string) => row.querySelector<HTMLInputElement | HTMLSelectElement>(`[data-field="${field}"]`); const stage = current.stages[index]; if (!stage) return; const minutes = get('threshold-minutes')?.value ?? ''; const seconds = get('threshold-seconds')?.value ?? ''; const parsedMinutes = minutes === '' ? 0 : Number(minutes); const parsedSeconds = seconds === '' ? 0 : Number(seconds); stage.name = get('name')?.value ?? stage.name; stage.threshold = Number.isInteger(parsedMinutes) && Number.isInteger(parsedSeconds) && parsedMinutes >= 0 && parsedSeconds >= 0 && parsedSeconds < 60 ? parsedMinutes * 60 + parsedSeconds : 0; stage.color = get('color')?.value ?? stage.color; stage.blink = (get('blink') as HTMLInputElement | null)?.checked ?? false; stage.buzzer = (get('buzzer')?.value ?? stage.buzzer) as Stage['buzzer']; }); const lastStageTime = current.stages[current.stages.length - 1]?.threshold ?? 0; const durationInput = value('#duration') ?? ''; const durationParts = durationInput.split(':'); const duration = durationParts.length === 2 && durationParts.every((part) => /^\d+$/.test(part)) ? Number(durationParts[0]) * 60 + Number(durationParts[1]) : 0; current.duration = Math.max(duration, lastStageTime + 30); }
function validCurrent(): boolean { return Boolean(current.name.trim() && current.speaker.trim() && current.club.trim()) && current.stages.length >= 3 && current.stages.every((stage, index) => Number.isFinite(stage.threshold) && stage.threshold >= 0 && (index === 0 || stage.threshold > current.stages[index - 1].threshold)); }
...
function stopAndSave(): void {
  ...
  history.unshift({ id: crypto.randomUUID(), presetTitle: run.preset.name || 'Untitled preset', speaker: speakerName(run.preset), club: clubName(run.preset), duration: endAt, startedAt, stoppedAt, savedAt: new Date().toISOString(), variance: endAt - stoppedAt, result, resultColor: run.preset.stages[stageIndex]?.color });
  persistHistory();
...
function exportHistoryCsv(): void {
  ...
  const rows = [['Preset title', 'Club name', 'Speaker name', 'Start date and time', 'Total duration', 'Saved stop time', 'Time remaining', 'Result', 'Saved at'], ...displayedHistory().map((item) => [item.presetTitle, item.club, item.speaker, item.startedAt ? new Date(item.startedAt).toISOString() : '', formatTime(item.duration), formatTime(item.stoppedAt), formatVariance(item.variance), item.result, item.savedAt])];
