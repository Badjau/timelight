...
type HistoryEntry = { id: string; presetTitle: string; speaker: string; duration: number; stoppedAt: number; savedAt: string; variance: number; result: string };
...
function stageMarkup(stage: Stage, index: number): string { const last = index === current.stages.length - 1; return `<article class="stage-row ${index === 0 ? 'is-expanded' : ''} ${last ? 'last-stage' : ''}" data-index="${index}" style="--stage-color:${stage.color}"><div class="stage-main"><span class="stage-number">${String(index + 1).padStart(2, '0')}</span><span class="stage-color" aria-hidden="true"></span><div class="stage-fields" id="stage-fields-${index}"><label>Stage stop output<input data-field="name" value="${escapeHtml(stage.name)}" maxlength="64" /></label><label>Starts at<input class="time-input" data-field="threshold" type="text" inputmode="numeric" maxlength="6" value="${formatTime(stage.threshold)}" /></label><label>Light color<div class="color-picker"><input data-field="color" type="color" value="${stage.color}" /><span>${stage.color}</span></div></label><label class="blink-field"><span>Blink </span><input data-field="blink" type="checkbox" ${stage.blink ? 'checked' : ''} /></label><label>Buzzer<select data-field="buzzer"><option value="none" ${stage.buzzer === 'none' ? 'selected' : ''}>No sound</option><option value="once" ${stage.buzzer === 'once' ? 'selected' : ''}>Chime once</option><option value="repeat" ${stage.buzzer === 'repeat' ? 'selected' : ''}>Repeat alert</option></select></label></div>${last ? `<div class="result-fields"><label>Over time result<input id="fail-result-output" maxlength="80" value="${escapeHtml(current.failResultOutput ?? 'Over time')}" /></label></div>` : ''}</div><div class="stage-summary"><button type="button" class="stage-toggle" data-stage-toggle aria-expanded="${index === 0}" aria-controls="stage-fields-${index}"><span class="stage-chevron" aria-hidden="true">&#8964;</span><span class="stage-summary-index">Stage ${index + 1}</span><strong class="stage-summary-name">${escapeHtml(stage.name || 'Untitled stage')}</strong><span class="stage-summary-threshold">${formatTime(stage.threshold)}</span><span class="stage-summary-color" aria-label="Light color ${stage.color}" title="Light color ${stage.color}"></span></button></div><div class="stage-actions"><button type="button" class="icon-button desktop-stage-action move-up" data-stage-action ${index === 0 ? 'disabled' : ''}>&uarr;</button><button type="button" class="icon-button desktop-stage-action move-down" data-stage-action ${last ? 'disabled' : ''}>&darr;</button><button type="button" class="icon-button desktop-stage-action remove-stage" data-stage-action ${current.stages.length <= 3 ? 'disabled' : ''}>&times;</button><button type="button" class="icon-button stage-menu-toggle" data-stage-menu-toggle aria-label="Stage actions" aria-expanded="false">&#8942;</button><div class="stage-menu" hidden><button type="button" class="stage-menu-action move-up" data-stage-action ${index === 0 ? 'disabled' : ''}>Move up</button><button type="button" class="stage-menu-action move-down" data-stage-action ${last ? 'disabled' : ''}>Move down</button><button type="button" class="stage-menu-action remove-stage" data-stage-action ${current.stages.length <= 3 ? 'disabled' : ''}>Delete stage</button></div></div></article>`; }
...
function historyRowsMarkup(): string { return history.length ? history.map((item) => `<tr><td>${escapeHtml(item.presetTitle)}</td><td>${escapeHtml(item.speaker)}</td><td>${formatTime(item.duration)}</td><td>${formatTime(item.stoppedAt)}</td><td>${formatVariance(item.variance)}</td><td>${escapeHtml(item.result)}</td></tr>`).join('') : '<tr><td colspan="6" class="history-empty">No stopped timers yet.</td></tr>'; }
function historyModalMarkup(): string { return `<div class="history-overlay" id="history-overlay" ${historyOpen ? '' : 'hidden'}><section class="history-card" role="dialog" aria-modal="true" aria-labelledby="history-title"><header><div><span class="zone-label">Saved timers</span><h2 id="history-title">History log</h2></div><button type="button" class="history-close" id="history-close" aria-label="Close history">&times;</button></header><div class="history-table-wrap"><table><thead><tr><th>Preset title</th><th>Speaker name</th><th>Total duration</th><th>Saved stop time</th><th>Time remaining</th><th>Result</th></tr></thead><tbody>${historyRowsMarkup()}</tbody></table></div><footer><span>${history.length} ${history.length === 1 ? 'entry' : 'entries'}</span><button type="button" class="secondary-button" id="export-history" ${history.length ? '' : 'disabled'}>Export CSV</button></footer></section></div>`; }
...
function render(): void {
  ...
  if (deleteButton) {
    ...
    deleteButton.insertAdjacentHTML('beforebegin', `<button type="button" class="toolbar-button icon-toolbar-button history-button" id="open-history" title="History log" aria-label="Open history log">${presetIcons.history}</button>`);
...
function moveStage(from: number, to: number): void { const fromThreshold = current.stages[from].threshold; const toThreshold = current.stages[to].threshold; [current.stages[from], current.stages[to]] = [current.stages[to], current.stages[from]]; current.stages[from].threshold = fromThreshold; current.stages[to].threshold = toThreshold; }
...
function selectPreset(presetId: string, fromTimer = false): void { const preset = presets.find((candidate) => candidate.id === presetId); if (!preset || (fromTimer && activeRun?.state === 'running')) return; current = structuredClone(preset); revertTarget = structuredClone(current); saved = true; if (fromTimer && activeRun) { activeRun = null; persistRun(); } render(); if (fromTimer) openLiveView(); }
function stopAndSave(): void {
  ...
  history.unshift({ id: crypto.randomUUID(), presetTitle: run.preset.name || 'Untitled preset', speaker: speakerName(run.preset), duration: run.preset.duration, stoppedAt, savedAt: new Date().toISOString(), variance: run.preset.duration - stoppedAt, result });
  persistHistory();
...
function bindEvents(): void {
  ...
  document.querySelector('#new-preset')?.addEventListener('click', () => { current = { id: crypto.randomUUID(), name: '', speaker: '', duration: 240, stages: structuredClone(defaultStages), failResultOutput: 'Over time', updatedAt: '' }; revertTarget = structuredClone(current); saved = false; render(); document.querySelector<HTMLInputElement>('#preset-name')?.focus(); });
