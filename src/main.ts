...
type HistoryEntry = { id: string; presetTitle: string; speaker: string; duration: number; stoppedAt: number; startedAt?: number; savedAt: string; variance: number; result: string; resultColor?: string };
...
function connectionMessage(): string { return navigator.onLine ? 'Online · Application shell is ready' : 'Offline · Running from the cached shell'; }
...
function stopAndSave(): void {
  ...
  history.unshift({ id: crypto.randomUUID(), presetTitle: run.preset.name || 'Untitled preset', speaker: speakerName(run.preset), duration: endAt, startedAt, stoppedAt, savedAt: new Date().toISOString(), variance: endAt - stoppedAt, result, resultColor: run.preset.stages[stageIndex]?.color });
  persistHistory();
...
function exportHistoryCsv(): void {
  ...
  const rows = [['Preset title', 'Speaker name', 'Start date and time', 'Total duration', 'Saved stop time', 'Time remaining', 'Result', 'Saved at'], ...displayedHistory().map((item) => [item.presetTitle, item.speaker, item.startedAt ? new Date(item.startedAt).toISOString() : '', formatTime(item.duration), formatTime(item.stoppedAt), formatVariance(item.variance), item.result, item.savedAt])];
