...
type HistoryEntry = { id: string; presetTitle: string; speaker: string; club: string; duration: number; allottedTime?: number; allottedTimeStart?: number; allottedTimeEnd?: number; stoppedAt: number; startedAt?: number; savedAt: string; variance: number; result: string; resultColor?: string };
const MAX_STAGES = 7;
...
function formatHistoryAllottedTime(item: HistoryEntry): string { return Number.isFinite(item.allottedTimeStart) && Number.isFinite(item.allottedTimeEnd) ? formatAllottedTimeRange({ start: item.allottedTimeStart!, end: item.allottedTimeEnd! }) : formatTime(item.allottedTime ?? item.duration); }
