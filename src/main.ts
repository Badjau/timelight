...
function bindEvents(): void {
  ...
  document.querySelector('#local-play')?.addEventListener('click', () => { startLocalTimer(); if (serial.status.state === 'connected') void handleDeviceCommand('start'); }); document.querySelector('#local-pause')?.addEventListener('click', () => { pauseLocalTimer(); if (serial.status.state === 'connected') void handleDeviceCommand('pause'); }); document.querySelector('#local-reset')?.addEventListener('click', () => { resetLocalTimer(); if (serial.status.state === 'connected') void handleDeviceCommand('reset'); }); document.querySelector('#local-next-stage')?.addEventListener('click', nextLocalStage);
