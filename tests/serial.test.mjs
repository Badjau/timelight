...
function fakePort({ ignoreFirstHello = false, retryOutput = false } = {}) {
  let controller;
  let helloCount = 0;
  let outputCount = 0;
  const writes = [];
  ...
  const writable = new WritableStream({ write(chunk) {
    ...
    if (message.type === 'hello') { helloCount++; if (ignoreFirstHello && helloCount === 1) return; controller.enqueue(new TextEncoder().encode(JSON.stringify({ version: 2, type: 'ready', requestId: message.requestId, firmware: 'test', ledCount: 116 }) + '\n')); }
    if (message.type === 'set_outputs') { outputCount++; if (retryOutput && outputCount === 1) return; controller.enqueue(new TextEncoder().encode(JSON.stringify({ version: 2, type: 'ack', requestId: message.requestId, appliedRevision: message.revision }) + '\n')); }
    if (message.type === 'keepalive') controller.enqueue(new TextEncoder().encode(JSON.stringify({ version: 2, type: 'ack', requestId: message.requestId, appliedRevision: 0 }) + '\n'));
  } });
...
test('handshake retries a missed boot message and malformed input remains recoverable', async () => {
  ...
  await serial.setOutputs({ revision: 1, color: '#0000ff', ledEffect: 'solid', transitionMs: 0, buzzerMode: 'none' });
  assert.equal(serial.status.state, 'connected');
...
test('acknowledgement timeout retries and output snapshots coalesce', async () => {
  ...
  const serial = new serialModule.ArduinoSerial(api);
  await serial.connect();
  const first = serial.setOutputs({ revision: 1, color: '#0000ff', ledEffect: 'solid', transitionMs: 0, buzzerMode: 'none' });
  const second = serial.setOutputs({ revision: 3, color: '#ff0000', ledEffect: 'blink', transitionMs: 450, buzzerMode: 'repeat' });
...
test('MCU reset re-handshakes, resynchronizes current outputs, and reconnects a replacement port', async () => {
  ...
  serial.onReady(() => {
    readyCount++;
    if (readyCount > 1) void serial.setOutputs({ revision: 7, color: '#ff0000', ledEffect: 'solid', transitionMs: 0, buzzerMode: 'none' });
  });
  await serial.connect();
  first.feed(JSON.stringify({ version: 2, type: 'ready', firmware: 'test-reset', ledCount: 116 }));
