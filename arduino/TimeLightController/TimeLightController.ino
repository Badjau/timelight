...
uint32_t desiredColor = 0, displayedColor = 0, transitionStartColor = 0, transitionTargetColor = 0, transitionElapsed = 0, blinkElapsed = 0, lastAnimationMillis = 0, lastLedFrameMillis = 0, nextBuzzerMillis = 0, oneShotEndsMillis = 0;
uint16_t transitionDuration = 0; LedEffect ledEffect = EFFECT_OFF; bool animationPlaying = false, oneShotPending = false; BuzzerMode buzzerMode = BUZZER_NONE;
...
void playOneShotBuzzer() { oneShotPending = true; }
void applyOutputs(uint32_t color, LedEffect effect, uint16_t transition, bool playing, BuzzerMode buzzer) { bool changed = effect != ledEffect || color != desiredColor, playOnce = buzzer == BUZZER_ONCE && playing && effect != EFFECT_OFF; if (effect == EFFECT_OFF) { desiredColor = 0; ledEffect = EFFECT_OFF; transitionDuration = transitionElapsed = blinkElapsed = 0; animationPlaying = false; setStripColor(0); } else { if (changed) { transitionStartColor = renderedColor(); desiredColor = transitionTargetColor = color; transitionDuration = transition; transitionElapsed = blinkElapsed = 0; } ledEffect = effect; animationPlaying = playing; } lastAnimationMillis = millis(); buzzerMode = buzzer == BUZZER_REPEAT ? BUZZER_REPEAT : BUZZER_NONE; nextBuzzerMillis = 0; if (buzzer == BUZZER_REPEAT || !playing || effect == EFFECT_OFF) { oneShotPending = false; oneShotEndsMillis = 0; noTone(BUZZER_PIN); } updateLeds(); if (playOnce) playOneShotBuzzer(); }
void updateBuzzer() { uint32_t now = millis(); if (oneShotPending) { oneShotPending = false; Serial.flush(); now = millis(); oneShotEndsMillis = now + BUZZER_ONCE_DURATION_MS; tone(BUZZER_PIN, 2400); return; } if (oneShotEndsMillis) { if ((int32_t)(now - oneShotEndsMillis) < 0) return; oneShotEndsMillis = 0; noTone(BUZZER_PIN); } if (buzzerMode != BUZZER_REPEAT) return; if (!nextBuzzerMillis || (int32_t)(now - nextBuzzerMillis) >= 0) { tone(BUZZER_PIN, 2400, BUZZER_REPEAT_DURATION_MS); nextBuzzerMillis = now + BUZZER_INTERVAL_MS; } }
...
void showStage(uint8_t index, bool alert) { StoredStage& stage = preset.stages[index]; BuzzerMode sound = (BuzzerMode)(stage.flags & 3), outputSound = timerState != TIMER_RUNNING ? BUZZER_NONE : sound == BUZZER_REPEAT ? BUZZER_REPEAT : alert && sound == BUZZER_ONCE ? BUZZER_ONCE : BUZZER_NONE; applyOutputs(stage.color, stage.flags & 4 ? EFFECT_BLINK : EFFECT_SOLID, 0, timerState == TIMER_RUNNING, outputSound); }
void toggleStandalone() { if (timerState == TIMER_IDLE) { timerState = TIMER_RUNNING; accumulatedMs = 0; runStartedMillis = millis(); standaloneStage = 0; showStage(0, true); } else if (timerState == TIMER_RUNNING) { accumulatedMs = standaloneElapsed(); timerState = TIMER_PAUSED; animationPlaying = false; buzzerMode = BUZZER_NONE; oneShotPending = false; oneShotEndsMillis = 0; noTone(BUZZER_PIN); } else { timerState = TIMER_RUNNING; runStartedMillis = millis(); animationPlaying = true; buzzerMode = (preset.stages[standaloneStage].flags & 3) == BUZZER_REPEAT ? BUZZER_REPEAT : BUZZER_NONE; nextBuzzerMillis = 0; } }
