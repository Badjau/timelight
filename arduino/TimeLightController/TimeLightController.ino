...
void sendReady(uint32_t requestId = 0) { uint8_t payload[7]; write16(payload, LED_COUNT); payload[2] = 0; payload[3] = 5; payload[4] = 3; payload[5] = 7; payload[6] = 1; sendFrame(FRAME_READY, requestId, payload, sizeof(payload)); }
...
void playOneShotBuzzer() { oneShotEndsMillis = millis() + BUZZER_ONCE_DURATION_MS; tone(BUZZER_PIN, 2400); }
void applyOutputs(uint32_t color, LedEffect effect, uint16_t transition, bool playing, BuzzerMode buzzer) { bool changed = effect != ledEffect || color != desiredColor, playOnce = buzzer == BUZZER_ONCE && playing && effect != EFFECT_OFF; if (effect == EFFECT_OFF) { desiredColor = 0; ledEffect = EFFECT_OFF; transitionDuration = transitionElapsed = blinkElapsed = 0; animationPlaying = false; setStripColor(0); } else { if (changed) { transitionStartColor = renderedColor(); desiredColor = transitionTargetColor = color; transitionDuration = transition; transitionElapsed = blinkElapsed = 0; } ledEffect = effect; animationPlaying = playing; } lastAnimationMillis = millis(); buzzerMode = buzzer == BUZZER_REPEAT ? BUZZER_REPEAT : BUZZER_NONE; nextBuzzerMillis = 0; if (buzzer == BUZZER_REPEAT || !playing || effect == EFFECT_OFF) { oneShotEndsMillis = 0; noTone(BUZZER_PIN); } updateLeds(); if (playOnce) playOneShotBuzzer(); }
void updateBuzzer() { uint32_t now = millis(); if (oneShotEndsMillis) { if ((int32_t)(now - oneShotEndsMillis) < 0) return; oneShotEndsMillis = 0; noTone(BUZZER_PIN); } if (buzzerMode != BUZZER_REPEAT) return; if (!nextBuzzerMillis || (int32_t)(now - nextBuzzerMillis) >= 0) { tone(BUZZER_PIN, 2400, BUZZER_REPEAT_DURATION_MS); nextBuzzerMillis = now + BUZZER_INTERVAL_MS; } }
